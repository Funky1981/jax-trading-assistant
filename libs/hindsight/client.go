// Package hindsight provides a client for the Hindsight memory service.
// Extracted from services/jax-memory/internal/infra/hindsight so that
// cmd/research can also use it directly (as part of ADR-0012 Phase 6
// consolidation of jax-memory into the research runtime).
package hindsight
...
// Client is an HTTP client for the Hindsight memory service.
type Client struct {
	baseURL    *url.URL
...
// New creates a Client for the given base URL (e.g. "http://hindsight:8888").
...
// Ping checks reachability of the hindsight service.
...
// Retain stores a memory item in the given bank.
...
// Recall retrieves memory items from the given bank.
...
// Reflect generates a synthetic memory reflection over stored items.
func (c *Client) Reflect(ctx context.Context, bank string, params contracts.ReflectionParams) ([]contracts.MemoryItem, error) {
	...
	reqBody := reflectRequest{Query: queryText}
