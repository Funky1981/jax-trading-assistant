// Command val03r-recovery-oos contains the future VAL-03R4 recovery execution
// pipeline. R3A exposes only contract-audit and preflight modes; execution is
// fail-closed until a separately authorized R4 run.
package main
...
func run() error {
	if len(os.Args) != 2 {
		return errors.New("usage: val03r-recovery-oos --contract-audit|--preflight-only|--execute")
	}
	switch os.Args[1] {
	case "--contract-audit", "--preflight-only", "--execute":
		return runR3A(os.Args[1])
	default:
		return errors.New("unsupported mode; recovery performance requires explicit VAL-03R4 authorization")
	}
}
...
func buildSignal(d *instrumentData, i, fast, medium, slow int, cfg FrozenExperimentConfig) (signal, string) {
	...
	s := signal{Date: d.Dates[i], Type: "HOLD", Confidence: 0, SMA20: sf, SMA50: sm, SMA200: ss, ATR14: atr}
	...
	if sf > sm && sm > ss {
		priceDiffPct := (price - sf) / sf
		if priceDiffPct >= -0.02 && priceDiffPct <= 0.01 {
			s.Type = "BUY"
			s.Confidence = 0.55
			s.Stop = price - 1.5*atr
			s.Target = price + 2*atr
			return s, "BUY"
		}
	}
	return s, "HOLD"
