...
function PersistedJourney({ evidence }: { evidence: Awaited<ReturnType<typeof operatorEvidenceService.candidate>> }) {
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>Paper ticket — hypothetical only</CardTitle><CardDescription>HYPOTHETICAL — NOT A FILL. No value below is an order, fill, position, account balance, or realised profit.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Fact label="Target" value={formatCurrency(evidence.target)} /><Fact label="Quantity" value={evidence.quantity?.toString() || 'Not supplied'} /><Fact label="Notional" value={formatCurrency(evidence.notional)} /><Fact label="Planned risk" value={formatCurrency(evidence.plannedRisk)} />
      <Fact label="Planned reward" value={formatCurrency(evidence.plannedReward)} /><Fact label="Reward/risk" value={evidence.rewardRisk === undefined ? 'Not supplied' : `${evidence.rewardRisk.toFixed(2)}R`} /><Fact label="Leverage" value={evidence.leverage === undefined ? 'Not supplied' : `${evidence.leverage.toFixed(2)}x`} /><Fact label="Account-equity assumption" value={formatCurrency(evidence.accountEquityAssumption)} /><Fact label="Fill state" value="NO FILL OCCURRED" />
    </CardContent></Card>
