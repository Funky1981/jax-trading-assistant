...
If `jax-trader` is not ready because no approved strategy artifacts are loaded, the frontend API may still answer `:8081` health checks, but runtime readiness on `:8100/ready` can fail. See `Docs/OPERATIONS/OPERATIONS.md` under "Trader has no approved artifacts".
