...
3. Use provenance filters or the persisted `NO_TRADE`, `WATCH`, and `CANDIDATE` decision filters to narrow the list.
...
7. Open `Decision` to inspect the persisted outcome, decision time, deterministic ruleset and processor, reasons, blockers, missing evidence, affected assets, trust/risk states, candidate linkage, and any version history. Open `Journey` to see what Jax did next.
...
### Understand evidence decisions

- `NO_TRADE` means Jax completed deterministic processing and found no justified reason to continue market review. This is a normal successful outcome.
- `WATCH` means the event may matter, but evidence such as truthful asset mapping, corroboration, a complete structured candidate, trust review, or risk review is still missing.
- `CANDIDATE` means every required persisted rule passed and an already-complete structured candidate is linked. It does not mean approved, ticketed, executed, ordered, traded, or filled.
- `Awaiting processing` remains visible only when no persisted decision exists. The browser never invents a decision from an inbox status.

Most evidence should be rejected or watched. More candidates are not considered better. Replay remains an operator-only command and the beginner UI is read-only.
