...
func (b *InstructionBuilder) NextPending(ctx context.Context, workerStartedAt time.Time) (*ApprovedInstruction, error) {
	...
	err := row.Scan(
		&inst.InstructionID, &inst.ApprovalID, &inst.CandidateID, &inst.SignalID,
...
// MarkSubmitted updates the instruction row to 'submitted' with trade and broker linkage.
func (b *InstructionBuilder) MarkSubmitted(ctx context.Context, instructionID uuid.UUID, tradeID, brokerOrderID string) error {
	_, err := b.pool.Exec(ctx, `
		UPDATE execution_instructions
		   SET status = 'submitted', trade_id = $2, broker_order_id = $3, submitted_at = NOW(), updated_at = NOW()
		 WHERE id = $1`, instructionID, tradeID, brokerOrderID)
	return err
}
...
func (b *InstructionBuilder) MarkCancelled(ctx context.Context, instructionID uuid.UUID, reason string) error {
	_, err := b.pool.Exec(ctx, `
		UPDATE execution_instructions
		   SET status = 'cancelled', error_message = $2, updated_at = NOW()
		 WHERE id = $1`, instructionID, reason)
