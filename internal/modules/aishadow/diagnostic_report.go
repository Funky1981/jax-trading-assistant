...
func DiagnosticReportMarkdown(report DiagnosticRunReport) string {
	...
	if hosted := report.HostedExperiment; hosted != nil {
		fmt.Fprintf(&builder, "## Hosted experiment\n\n- Cell: %s\n- Provider / requested model: %s / %s\n- Returned model identities: %s\n- Reasoning: %s\n- Structured output mode: %s\n- Requests / corrective retries: %d / %d\n- Input / output tokens: %d / %d\n- Calculable / ambiguous / ceiling USD: %s / %s / %s\n- Stop reason: %s\n\n", hosted.ExperimentID, hosted.Provider, hosted.RequestedModel, strings.Join(hosted.ReturnedModels, ", "), hosted.ReasoningEffort, hosted.StructuredOutputMode, hosted.RequestCount, hosted.RetryCount, hosted.Usage.InputTokens, hosted.Usage.OutputTokens, hosted.ActualCalculableCostUSD, hosted.AmbiguousLiabilityUSD, hosted.BudgetCeilingUSD, hosted.StopReason)
	}
